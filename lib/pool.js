...
function BlockTemplate(template){
    ...
    if (config.poolServer.extraNonce) {
...
function getBlockTemplate(callback){
    if(config.poolServer.extraNonce) {
        ...
        apiInterfaces.rpcDaemon('getblocktemplate', {wallet_address: config.poolServer.poolAddress}, callback);
    }
}
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                if(config.poolServer.extraNonce) {
                    ...
                    var blockFastHash = cryptoNightFast(convertedBlob || shareBuffer).toString('hex');
                }
                log('info', logSystem,
